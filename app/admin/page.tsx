      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 20, alignItems: "center", marginBottom: 30 }}>
        <div>
          ...
          <p style={{ marginBottom: 0, color: "#66736b" }}>
            You can correct the winning name or amount below, then press <strong>Mark sold</strong> when the live bidding finishes.
          </p>
        </div>
          ...
          <section key={lot.id} style={{ border: lot.active ? "1px solid #ddd" : "2px solid #9c2f2f", borderRadius: 12, padding: 20, marginBottom: 20, background: lot.active ? "white" : "#fff7f7" }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 20, alignItems: "center", flexWrap: "wrap" }}>
              <div>
                ...
                <p style={{ marginTop: 0 }}>
                  <strong style={{ color: lot.active ? "#195633" : "#9c2f2f" }}>{lot.active ? "OPEN" : "SOLD"}</strong>
                  {winningBid && <> · {lot.active ? "Current highest bid" : "Sold for"} <strong>{money(winningBid.amountPence)}</strong> by <strong>{winningBid.bidderName}</strong></>}
                </p>
              </div>
              <form action={toggleLot}>
                <input type="hidden" name="lotId" value={lot.id} />
                <input type="hidden" name="active" value={String(lot.active)} />
                <button type="submit">{lot.active ? "Mark sold" : "Reopen lot"}</button>
              </form>
            </div>
            ...
            {winningBid && (
              <form action={editWinningBid} style={{ display: "flex", gap: 10, alignItems: "end", flexWrap: "wrap", padding: "12px 0 16px", borderTop: "1px solid #eee", marginTop: 8 }}>
                ...
                <label style={{ display: "grid", gap: 4, fontWeight: 700 }}>
                  Winner
                  <input name="bidderName" defaultValue={winningBid.bidderName} required style={{ padding: 9, minWidth: 180 }} />
                </label>
                <label style={{ display: "grid", gap: 4, fontWeight: 700 }}>
                  Winning bid (£)
                  <input name="amount" type="number" min="1" step="1" defaultValue={winningBid.amountPence / 100} required style={{ padding: 9, width: 130 }} />
                </label>
            ...
            {lot.bids.length === 0 ? <p>No bids yet.</p> : (
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead><tr><th align="left">Bidder</th><th align="left">Bid</th><th align="left">Time</th><th></th></tr></thead>
                  <tbody>
                    {lot.bids.map((bid) => (
                      <tr key={bid.id}>
                        <td style={{ padding: "10px 0" }}>{bid.bidderName}</td>
                        <td>{money(bid.amountPence)}</td>
                        <td>{bid.createdAt.toLocaleString("en-GB", { timeZone: "Europe/London", day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })}</td>
                        <td align="right"><form action={deleteBid}><input type="hidden" name="bidId" value={bid.id} /><button type="submit">Delete</button></form></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
